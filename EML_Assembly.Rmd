...
diet_freq <- diet_EDI %>%
  select(vernacularName, preyTaxon, preyCount) %>%
  group_by(vernacularName, preyTaxon) %>%
  summarise(preytotal = sum(preyCount)) %>%
  mutate(preyfreq = 100*(preytotal/sum(preytotal))) 

# pull out example
diet_freq %>% filter(preyTaxon == "PPC")
# double check: make sure the prey frequency adds up to 100% for every fish 
diet_freq %>% filter(vernacularName == "Alewife") %>%
  summarise(total_freq = sum(preyfreq))
...
diet_occur <- diet_EDI %>%
  ...
  group_by(preyTaxon) %>%
  add_tally(name = "preycount") %>%
  mutate(freq = (preycount/389)*100)
...
North <- round(max(diet_EDI$decimalLatitude), 5)
East <- round(max(diet_EDI$decimalLongitude), 5)
South <- round(min(diet_EDI$decimalLatitude), 5)
West <- round(min(diet_EDI$decimalLongitude), 5)
