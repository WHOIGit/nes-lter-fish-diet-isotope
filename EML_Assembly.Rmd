...
unique(diet_long$uniqueIdentifierFish)
# There's 501 unique fish identifiers
...
```{r}
str(isotope_EDI)
var(isotope_EDI$d13C)
var(isotope_EDI$d15N)
var(isotope_EDI$C_to_N)
var(isotope_EDI$d13C_corr)
...
iso_check <- isotope_EDI %>%
  ...
  summarise(mean_d13C = mean(d13C),
            ...
            # ymin = (mean(d13C) - std.error(d13C)),
            # ymax = (mean(d13C) + std.error(d13C)))
...
Write a generalized function for replacing a node of xml
...
proj <- read_xml("project.txt", from = "xml")

iso_eml <- read_eml(paste0(dir, "/fish-isotope/knb-lter-nes.3.1.xml"), from = "xml")
iso_proj <- iso_eml[[4]]$project
eml_validate(iso_eml)

xml_add_parent(iso_eml, iso_proj)
