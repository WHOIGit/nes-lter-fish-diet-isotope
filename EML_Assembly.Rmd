...
IN PROGRESS: Code will be generalized to for replacing any chosen node of an xml file

Step 1: Read in the xml file and the replacement xml chunk
Step 2: Delete/replace the node of interest
...
proj_eml <- read_xml("parent_project.txt", from = "xml")

# read in diet xml file exported by make_eml
diet_eml <- read_xml("https://raw.githubusercontent.com/WHOIGit/nes-lter-fish-diet-isotope/master/fish-diet/knb-lter-nes.2.1.xml", from  = "xml")
# both objects should be of class c("xml_document" "xml_node")

# isolate old node
diet_children <- xml_children(diet_eml)
oldnode <- xml_children(diet_children[[2]])[[15]] # loc 15 is the project node
# designate new node
newnode <- proj_eml
# replace project node
xml_replace(oldnode, newnode)

# check that the node was replaced
xml_children(diet_children[[2]])[[15]]
# check that the file is still valid
eml_validate(diet_eml)

# write modified xml file
write_xml(diet_eml, paste0(dir, "/fish-diet/knb-lter-nes.2.2.xml"))
