...
diet_join <- diet_join[complete.cases(diet_join[ , "GMT_DAY"]),]
...
# Fish
# Load stomach content data from excel sheet
fishkey <- read_excel(path = paste0(dir, "/", diet_xls), 
                  sheet = "FishLookup", na = "NA")
...
diet_EDI <- diet_final %>%
  select(-Time, -GMT_YEAR, -GMT_MONTH, -GMT_DAY, - preytype_stnd)
...
# merge geospatial and temporal columns from diet data 
isotope_EDI <- unique(left_join(x = isotope, y = diet_EDI %>%
                             select(uniqueIdentifierFish, decimalLatitude, 
                                    decimalLongitude, time_UTC, date, 
                                    scientificName_fish, scientificNameID_fish, vernacularName),
                          by = "uniqueIdentifierFish"))

# isolate missing fisheries cruise_stations (75 entries...email Justin)
isotope_missing <- isotope_EDI[is.na(isotope_EDI$date),]
# write_csv(isotope_missing,"ISOTOPE_MIA.csv")
# check if valid haul in Fisheries datasheets
# eliminate rows with missing fisheries data (40 total)
...
colnames(isotope_EDI)[colnames(isotope_EDI)=="dC corr"] <- "dC_corr"
...
tz_test <- diet_join %>% select(time_local, GMT_TIME)
```
...
```
The data fails the visual check so we have to fix that...

## Installing EML Assembly from GitHub
...
# Determine start and end date (YYYY-MM-DD)
# Used for make_eml
...
## Categorical Vars --------
# Define categorical variables for diet data
define_catvars(path = paste0(dir,"/fish-diet"))
# Region is the only catvar, the expanded regions were defined manually

## Geographic coverage --------
# define bounding box for all observations
...
## Categorical Vars --------
# there are no categorical variable for this dataset
